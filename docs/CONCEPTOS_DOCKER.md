...
| `EXPOSE 8004` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
- En la v1: `bd_v1` es un contenedor creado desde la imagen `postgres:16-alpine`,
  con el puerto interno 5432 **publicado** en el 15434 de su PC (`-p 15434:5432`).
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8004"]
    ...
    NAV -->|"localhost:8004"| APIFACTURAS
    APIFACTURAS -->|"mariadb:3306 (DNS de Docker)"| MARIADB
    APIFACTURAS -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13334"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15434"| POSTGRES
```
...
services:                          # el mapa de TODOS los contenedores del sistema
  ...
  postgres:                        # ← este nombre es también su HOSTNAME interno
    ...
    ports:
      - "15434:5432"               # "puerto en su PC : puerto interno del contenedor"
  ...
  api-facturas:
    ...
    ports:
      - "8004:8004"                # http://localhost:8004/docs
    environment:
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8004`, `localhost:15434`); entre contenedores, nombres de
...
docker network create proyecto_paradigmas3_default

# 2. postgres
docker run -d --name postgres --network proyecto_paradigmas3_default --restart unless-stopped `
...
# 4. mariadb
docker run -d --name mariadb --network proyecto_paradigmas3_default --restart unless-stopped `
  -e "MARIADB_ROOT_PASSWORD=paradigmas123" `
  -v mariadbdata:/var/lib/mysql `
  -v "${PWD}/db/init_mariadb.sql:/docker-entrypoint-initdb.d/init_mariadb.sql:ro" `
  -p 13334:3306 mariadb:11
...
docker run -d --name api-facturas --network proyecto_paradigmas3_default --restart unless-stopped `
  -e "DB_POSTGRES=postgresql+asyncpg://paradigmas:paradigmas123@postgres:5432/bdfacturas_postgres_local" `
  -e "DB_MARIADB=mysql+aiomysql://root:paradigmas123@mariadb:3306/bdfacturas_mariadb_local" `
  -e "DB_PROVIDER=${DB_PROVIDER:-postgres}" `
  -v "${PWD}/api_facturas:/app" `
  -p 8004:8004 api-facturas uvicorn main:app --host 0.0.0.0 --port 8004 --reload
...
```yaml
URL_API: http://api-facturas:8004
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8004` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8004` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8004:8004`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8004`, el front
...
```powershell
docker network inspect proyecto_paradigmas3_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_paradigmas3` produce
la red `proyecto_paradigmas3_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_paradigmas3-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_paradigmas3_default has active endpoints (name:"proyecto_paradigmas3-api-facturas-1" …)
```
